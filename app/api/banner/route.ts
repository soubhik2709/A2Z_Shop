  ...
  try {
    ...
    const slides = await Promise.all(
      ...
      SLIDE_CONFIG.map(async (config) => {
        ...
        const res = await fetch(searchUrl, { next: { revalidate: 172800 } }); //2 days
        ...
        const photo = data.results?.[0]; // Get the first matching photo for checking
    ...
    return NextResponse.json(slides, {
      // what is this headers?
      headers: {
        "Cache-Control": "public, s-maxage=172800, stale-while-revalidate=1800",
      },
    });
  } catch (error) {
